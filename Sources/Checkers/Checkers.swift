 ...
 *
 * Version: 1.1.10
 * Originally created by Ky on 2010/02/15
 * Latest Java version finished on 2010/04/13
 * This file rewritten in Swift for funzies on 2025-11-14
 *
 * Copyright Ky, © 2010 under Creative Commons 3.0 (CC-BY-SA)
 * Copyright Ky, © 2025 under The Fair License
 */
import Foundation
import Introspection
import SimpleLogging
@main struct Checkers {
  //Declarations:
  private static var computer = false, compVsComp = false, delimHit = Bool(), doubleJumping = false, move = true, named1 = false, named2 = false, quit = false, turnOver = false, winner = false
  private static var mark = Character(), notMark = Character(), mark1: Character = "b", mark2: Character = "r", `where` = Character(), EMPTY: Character = " "
  private static var board: [[Character]] = []
  private static var turns = Int(), to1 = Int(), to2 = Int(), from1 = Int(), from2 = Int(), pieces1 = Int(), pieces2 = Int(), piece = Int(), notPiece = Int(), p1wins = Int(), p2wins = Int(), numOfPlayers = 2
  private static var priority: [[Int]] = []
  private static var printer: LogChannel = .errorFile
  private static var color = String?.none, color1 = "black", color2 = "red", notColor = String?.none, name = String?.none, name1 = Introspection.username, name2 = "Player 2", notName = String?.none, /*square = "", */ spareStr = "", error = String?.none, header = String?.none;
  private static let SIZE: Int = /*Integer.parseInt(JOptionPane.showInputDialog(null, "size?", */8/*))*/;
  private static let TITLE = GameProps.getProp("title"), VERSION = GameProps.getProp("version"), SAVEFILE = "CheckersSave.dll", ERROR_FILE = "errors.txt";
  
  public static func main() throws
  {
    //Begin error reporting
    LogManager.defaultChannels.append(.errorFile)
    log(error: """
            This is the error-reporting file for \(TITLE) (version \(VERSION))
            
            
                           program run from: " + System.getProperty("user.dir")
            program used to run the program: " + System.getProperty("java.class.path")
                                    OS name: " + System.getProperty("os.name") + " (version " + System.getProperty("os.version") + ")"
                            OS Architecture: " + System.getProperty("os.arch")
              Java Virtural Machine version: " + System.getProperty("java.vm.version")
  ...
  //    System.setOut(new PrintStream(new File("cmd.exe")));
    ...
    /*square = "";*/spareStr = "";
    priority = .init(repeating: .init(repeating: .init(), count: SIZE), count: SIZE)
    board = .init(repeating: .init(repeating: .init(.init()), count: SIZE), count: SIZE)
    ...
    {//*TC*/System.out.println(1);
      ...
      pieces2 = 0;
      //
      for i in 0 ..< SIZE //SETS ALL SQUARES AS EMPTY
      {
        for j in 0 ..< SIZE {
          board[i][j] = (EMPTY); }
      }
      log(error: "Board initialized successfully");

      for i in 0 ..< (SIZE / 2) - 1 //SETS PLAYER 2'S PIECES
      {
        var j = 1; while j < SIZE
        {defer{j++; j++}
          if (j == 1 && i == 1) {
            j--;}
          board[i][j] = mark2;
      ...
      log(error: "Player 2's pieces cuccessfully placed");

      for i in stride(from: SIZE, to: SIZE - ((SIZE / 2) - 1), by: -1) //SETS PLAYER 1'S PIECES
      {
      ...
      log(error: "Player 1's pieces cuccessfully placed");
      ...
      {//*TC*/System.out.println(2);
        ...
        turns=0; while (!quit && pieces1 > 0 && pieces2 > 0)
        {defer{turns++}//*TC*/System.out.println(3);
...
//            owner = false;
          ...
          print(GameBoard.print(SIZE, board));
          log(error: GameBoard.miniPrint(board));
          //END BOARD CREATION
      ...
      print(GameBoard.print(SIZE, board), terminator: "");
      ...
      if (turns == ((SIZE * SIZE) - 1) && !winner) {
        JOptionPane.showMessageDialog(nil, "Sorry... no one wins. :/", header + " - " + TITLE, 1);}
      else if (move)
      {
        JOptionPane.showMessageDialog(nil, name1 + " wins!", header + " - " + TITLE, 1);
        p1wins++;
      }
      else
      {
        JOptionPane.showMessageDialog(nil, name2 + " wins!", header + " - " + TITLE, 1);
        p2wins++;
      }
      ...
      {
        log(error: "User opted to NOT play again");
        if (YesNoBox.bool("Would you like to save your score?", TITLE, 3))
        {
          log(error: "User opted to save score");
          do
          {
            quit = false;
            ...
            if (named1 && named2)
            {
              log(error: "Usernames have been entered previously, saving scores with " + name1 + " and " + name2 + ".");
  ...
  ////////////////////////////////////////////////////////USEFUL METHODS////////////////////////////////////////////////////////
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  private static func countPieces()
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  public static func computerTurn()
  {//*TC*/System.out.println("BEGIN computerTurn()");
    ...
    for i in 0 ..< SIZE && !turnOver
    {
      for j in 0 ..< SIZE && !turnOver
      {
        if (priority[i][j] >= 3)
        {
          from1 = i;
